...
public:
  ...
  sc_signal< sc_int<24> >  dsp0;
  sc_signal< sc_int<24> >  dsp1;  
  sc_signal<bool> 	 dsp_tick;
  sc_signal<bool>        mclk_mux;
  sc_signal<bool>        mrst_n;  
  sc_signal<bool> 	 mtick;
  sc_signal<bool> 	 mplay;
  sc_signal<bool> 	 mreq;
  sc_signal<bool> 	 mcfg;   
  sc_signal< sc_int<24> >  mdsp0;
  sc_signal< sc_int<24> >  mdsp1;
  sc_signal< sc_uint<32> > mcfg_reg;
  sc_signal<bool> 	 req;
#endif
  ...
  clk("clk"),
...
#endif
    {
      ...
      dsp_unit_1.dsp0_out(dsp0);
      dsp_unit_1.dsp1_out(dsp1);
      dsp_unit_1.valid_out(dsp_tick);

      cdc_unit_1.clk(clk);
      cdc_unit_1.rst_n(rst_n);
      cdc_unit_1.test_mode_in(test_mode_in);      
      cdc_unit_1.dsp0_in(dsp0);
      cdc_unit_1.dsp1_in(dsp1);      
      cdc_unit_1.tick_in(dsp_tick);
      cdc_unit_1.cfg_reg_in(cfg_reg);
      cdc_unit_1.cfg_in(cfg);
      cdc_unit_1.play_in(play);
      cdc_unit_1.req_out(req);
      cdc_unit_1.mclk_in(mclk);
      cdc_unit_1.mclk(mclk_mux);      
      cdc_unit_1.mrst_n(mrst_n);
      cdc_unit_1.dsp0_out(mdsp0);
      cdc_unit_1.dsp1_out(mdsp1);
      cdc_unit_1.tick_out(mtick);
      cdc_unit_1.cfg_reg_out(mcfg_reg);
      cdc_unit_1.cfg_out(mcfg);
      cdc_unit_1.play_out(mplay); 
      cdc_unit_1.req_in(mreq);

      i2s_unit_1.clk( mclk_mux );
      i2s_unit_1.rst_n( mrst_n );
      i2s_unit_1.play_in( mplay );
      i2s_unit_1.tick_in( mtick );
      i2s_unit_1.audio0_in( mdsp0);
      i2s_unit_1.audio1_in( mdsp1);
      i2s_unit_1.cfg_in( mcfg );
      i2s_unit_1.cfg_reg_in( mcfg_reg );
