...
filename="results/dsp_unit_vsim_rtl_out.txt"
...
let y=20
gnuplot  -geometry ${wt}x${ht}+${x}+${y} -persist  -e "set terminal x11 title \"abuf_in[0]\"; set style line 1 lc rgb \"green\"; set style data lines; plot \"${filename}\" using 1:${li} ls 1; pause -1" &
let y=20+${dy}
gnuplot  -geometry ${wt}x${ht}+${x}+${y} -persist -e "set terminal x11 title \"abuf_in[1]\"; set style line 1 lc rgb \"green\"; set style data lines; plot \"${filename}\" using 1:${ri} ls 1; pause -1" &
let y=20+2*${dy}
gnuplot  -geometry ${wt}x${ht}+${x}+${y} -persist -e "set terminal x11 title \"dsp_out[0]\"; set style line 1 lc rgb \"red\"; set style data lines; plot \"${filename}\" using 1:${lo} ls 1; pause -1" &
let y=20+3*${dy}
gnuplot  -geometry ${wt}x${ht}+${x}+${y} -persist -e "set terminal x11 title \"dsp_out[1]\"; set style line 1 lc rgb \"red\"; set style data lines; plot \"${filename}\" using 1:${ro} ls 1; pause -1" &
